...
def _(cond, norm, np, pl, solve):
    ...
    def analyze_matrix_family(
        n_values: list, matrix_generator, name: str
    ) -> pl.DataFrame:
...
def _(all_results, pl, plt):
    def generate_plots(results: pl.DataFrame) -> tuple[plt.Figure, plt.Figure]:
        ...
        for matrix_type in results["matrix_type"].unique():
            ...
            plt.semilogy(
                data["size"], data["cond_2norm"], "o-", label=matrix_type, markersize=6
            )
        ...
        plt.xlabel("Matrix Size $(n)$")
        plt.ylabel(
            r"Relative Error $|| \bm{x} - \bm{x}_{true}||_2 / |\bm{x}_{true} |_2$"
        )
    ...
    def print_detailed_analysis(results: pl.DataFrame) -> str:
        ...
        for matrix_type in results["matrix_type"].unique():
            ...
            if max_cond > threshold:
                analysis.append(
                    f"WARNING: Matrix becomes ill-conditioned (κ > {threshold:.1e})"
                )
                ...
                if not ill_cond_size.is_empty():
                    ...
                    analysis.append(
                        f"Becomes ill-conditioned at size n = {first_ill_cond}"
                    )
